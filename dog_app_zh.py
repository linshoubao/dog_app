...
# 
# __回答:__ 我是根据图中提示进行了搭建卷积网络，第一层添加了卷积层，运用了16个滤波器，卷积核2x2，步长为1，采用了不填充，也就是可以接受不完整图片，输入了224x224x3彩色像素；第二层添加了最大池化层，运用池化层主要是降低维度，防止过拟合，同时提供运算速率；第三层添加了卷积层，设置了32个滤波器，目的是刻画更多局部的边缘和纹理特征；第四层添加了最大池化层，减少参数；第五层添加了卷积层，设置了64个滤波器，刻画全局特征；第六层添加了最大池化层，同样的也是减少参数；第七层添加了全局平均池化层，目的也是降低参数，便于后面添加全连接层；第八层添加全连接层，使用133个节点对应133个类，同时添加了softmax激活函数，获取对应的概率。
...
# 
# 
# __回答:__ 第一次使用VGG19预训练模型，但是测试准确率只有百分之四十多，第二次使用Resnet50模型，准确率达到了百分之八十多，因此采用了Resnet50模型；迁移学习是机器学习技术的一种，其在一个任务上训练的模型被重新利用在另一个相关的任务上，核心是利用已调整好的网络结构和节点权重，建模时进展会变快或性能会提高；要注意的是预训练的神经网络，要和当前的任务差距不大，不然迁徙学习的效果会很差，也就是说用不同的预训练模型进行迁移学习效果不一样。
# 
