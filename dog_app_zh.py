...
for img_path in dog_files_short:
    if face_detector(img_path) ==True:
        count_dog +=1
print (count_human)
print ('human_files的前100张图像中，能够检测到人脸的图像占比:%s'%(count_human/len(human_files_short)*100))
print ('dog_files的前100张图像中，能够检测到人脸的图像占比:%s'%(count_human/len(dog_files_short)*100))
...
for i in range(100):
    ...
    if dog_detector(dog_files_short[i])==True:
        count_dog +=1
print ('human_files_short中图像检测到狗狗的百分比:%s' %(count_human/len(human_files_short)*100))
print ('dog_files_short中图像检测到狗狗的百分比:%s' %(count_dog/len(dog_files_short)*100))
