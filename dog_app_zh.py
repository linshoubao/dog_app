...
def detect(detector,files):
    return np.mean([detector(i) for i in files])

print ('human_files的前100张图像中，能够检测到人脸的图像占比:{:.2%}'.format(detect(face_detector,human_files_short)))
print ('dog_files的前100张图像中，能够检测到人脸的图像占比:{:.2%}'.format(detect(face_detector,dog_files_short)))
...
# 
# __回答:__现实中，提供清晰的面部特征并不合理的，拍摄的像素会影响人脸图像的质量。我们可以调整图像的亮度或者对比度来增加清洗的面部特征。
# 
# 
...
print ('human_files的前100张图像中，能够检测到人脸的图像占比:{:.2%}'.format(detect(dog_detector,human_files_short)))
print ('dog_files的前100张图像中，能够检测到人脸的图像占比:{:.2%}'.format(detect(dog_detector,dog_files_short)))
...
# 
# __回答:__ 我是根据图中提示进行了搭建卷积网络，第一层添加了卷积层，运用了16个滤波器，卷积核2x2，步长为1，采用了不填充，也就是可以接受不完整图片，输入了224x224x3彩色像素；第二层添加了最大池化层，运用池化层主要是用来只保留这些特征中最强的，保证特征的位置与旋转不变性，减少模型参数数量，减少过拟合问题，同时提供运算速率；第三层添加了卷积层，设置了32个滤波器，目的是提取越来越高级的特征；第四层添加了最大池化层，减少参数；第五层添加了卷积层，设置了64个滤波器，同理也是提取越来越高级的特征；第六层添加了最大池化层，同样的也是减少参数；第七层添加了全局平均池化层，目的也是降低参数，便于后面添加全连接层；第八层添加全连接层，使用133个节点对应133个类，同时添加了softmax激活函数，获取对应的概率。
...
# 
# 
# __回答:__ 1.第一次使用VGG19预训练模型，但是测试准确率只有百分之四十多，第二次使用Resnet50模型，准确率达到了百分之八十多，因此采用了Resnet50模型；2.迁移学习是机器学习技术的一种，其在一个任务上训练的模型被重新利用在另一个相关的任务上，核心是利用已调整好的网络结构和节点权重，建模时进展会变快或性能会提高；3.要注意的是预训练的神经网络，要和当前的任务差距不大，不然迁徙学习的效果会很差，也就是说用不同的预训练模型进行迁移学习效果不一样；4.这四个迁移模型是用利用ImageNet（手动标注的图片数据库）数据集训练的；5.第三步利用CNN,简单搭建的卷积神经网络架构，训练数据慢、准确率非常低；6.VGG16的缺点是网络架构weight数量相当大，很消耗磁盘空间，同时训练速度慢；7.四个迁移模型优点：一是ResNet是一种残差模块让我们可以训练更深的网络；二是VGGNet是全连接节点的数量较多，在较小的网络架构中通常更为理想；三是InceptionNet比VGG训练更快，占内存小；四是Xception是Inception架构的扩展，它用深度可分离的卷积代替了标准的Inception模块，weight权重数量更小。
# 
...
### TODO: 从另一个预训练的Resnet50获取bottleneck特征
bottleneck_features=np.load('/data/bottleneck_features/DogResnet50Data.npz')
train_Resnet50=bottleneck_features['train']
valid_Resnet50=bottleneck_features['valid']
test_Resnet50=bottleneck_features['test']
...
recognize_dog('tupian/alasijia.jpeg')


# In[66]:


recognize_dog('tupian/jingmao.jpg')


# In[67]:


recognize_dog('tupian/keji.jpg')


# In[68]:


recognize_dog('tupian/Bulldog.jpg')


# In[69]:


recognize_dog('tupian/zhangshaohan.jpg')


# In[70]:


recognize_dog('tupian/zhengkai.jpg')


# In[71]:


recognize_dog('tupian/maomi.jpg')


# 回答：1.总共进行了7张图片的测试，能够正确区分狗和人的图像，正确达到了100%，同时针对猫的图像能够无法识别，除机器识别是新斯科舍猎鸭寻猎犬，识别错误的原因可能是图像结果比我想象的要好；
# 
# 建议：1.模型拟合时，增加epochs次数，期望找出最佳验证loss的模型权重；
#       2.对训练集进行 数据增强 以优化模型的表现；
#       3.编译模型时，调整参数，找出最优参数。
