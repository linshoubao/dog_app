...
# 
# __回答:__ 我是根据图中提示进行了搭建卷积网络，运用了三次卷积层和三次最大池化层，运用池化层主要是降低维度，防止过拟合，同时提供运算速率，最后使用了极端措施-全局平均池化层，维度为1的64层筛选器，最后分类成133类，预测133种情况。
...
# 
# 
# __回答:__ 搭建网络架构使用了迁移学习增加了两个层：全局平均池化层，目的是降低训练的参数数量，提高训练时间；全连接层使用了133个节点，同时使用了softmax激活函数，将结果分类成133种
# 
# 
